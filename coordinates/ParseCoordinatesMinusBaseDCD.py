...
# Global variables
#-------------------------------------------------------------------------------------------------------
#mix = "Tetradecane100" # number of the mixture - e.g Pentane09
#workPath = "/media/jordancaraballo/Datos_Frances/jordancaraballo/Investigacion/Tetradecane/" + mix + "/"
mix = "Pentane100" # number of the mixture - e.g Pentane09
workPath = "/home/jordancaraballo/Investigacion/ENE-MAY-2018/Pentane/" + mix + "/"
...
mid_type    = 'C8'        # atom that connects pABA and Cellulose, pentane is C8
...
# Function: getIndexUppAndMidTypes(frames, pdb_list, psf_list, upper_types, mid_type)
    frames - dcd file
    bl;ablaklabnkank
# Iterate over each trajectory and over each line in the pdb to find the indexes
# where the atoms are located. This indexes are based on the psf and pdb lists,
# not on the lines directly from the file. For example:
            ...
            try:
                ...
                if psf_list[atom][3] == 'PAB':
                    ...
                    if (psf_list[atom][5])[:len(upper_types)] in upper_types:
                        uppTypePos.append(int(pdb_list[atom][1])) # store position that has in the pdb list
                    if (psf_list[atom][5])[:len(mid_type)] == mid_type:
                        contPABA += 1
                        midTypePos.append(int(pdb_list[atom][1])) # store position that has in the pdb list
            except IndexError:
                pass
        break
